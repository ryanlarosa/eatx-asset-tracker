...
const RepairTickets: React.FC = () => {
    ...
    const getStatusColor = (s: string) => {
        switch(s) {
            case 'New': return 'bg-purple-50 text-purple-700 border-purple-100';
            case 'Open': return 'bg-red-50 text-red-600';
            case 'In Progress': return 'bg-blue-50 text-blue-600';
            case 'Waiting for Parts': return 'bg-amber-50 text-amber-600';
    ...
    return (
        <div className="space-y-6">
            <header className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
                        <Wrench className="text-slate-900"/> Maintenance & Repairs
                    </h1>
                    <p className="text-slate-500 text-sm">Report broken devices and track repair progress.</p>
                </div>
                <div className="flex gap-2">
                    <button onClick={() => navigator.clipboard.writeText(publicLink)} className="bg-white border border-slate-200 text-slate-700 px-3 py-2 rounded-lg hover:bg-slate-50 flex items-center gap-2 text-sm font-medium">
            ...
            {/* Create Modal */}
             {isCreating && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6 border border-slate-100 animate-in fade-in zoom-in max-h-[90vh] overflow-y-auto">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-lg font-bold text-slate-900">New Incident Report</h3>
                            <button onClick={() => setIsCreating(false)}><X size={20} className="text-slate-400"/></button>
                        </div>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">1. Location</label>
                                <select 
                                    required 
                                    className="w-full p-2.5 border border-slate-300 rounded-lg bg-slate-50 focus:ring-2 focus:ring-slate-900"
                            ...
                            {newTicket.location && (
                                <div className="animate-in slide-in-from-top-2 space-y-4">
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1">2. Asset (Optional)</label>
                                        <select 
                                            className="w-full p-2.5 border border-slate-300 rounded-lg bg-slate-50 focus:ring-2 focus:ring-slate-900"
                                    ...
                                    <div className={`space-y-3 p-3 bg-slate-50 rounded-lg border border-slate-100 ${newTicket.assetId ? 'opacity-50 pointer-events-none' : ''}`}>
                                        <div>
                                            <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Device Name / Desc</label>
                                            <input 
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            <div>
                                                ...
                                                <select required={!newTicket.assetId} className="w-full p-2 border border-slate-300 rounded-lg text-sm bg-white" value={newTicket.deviceType} onChange={e => setNewTicket(prev => ({...prev, deviceType: e.target.value}))}>
                                            ...
                                            <div>
                                                ...
                                                <input placeholder="Optional" className="w-full p-2 border border-slate-300 rounded-lg text-sm" value={newTicket.reportedSerial} onChange={e => setNewTicket(prev => ({...prev, reportedSerial: e.target.value}))}/>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">3. Issue Description</label>
                                <textarea 
                                    required
                                    rows={3}
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-3">
                                    <input type="file" accept="image/*" onChange={handleImageUpload} className="text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200"/>
                            ...
                            <div className="pt-4 flex gap-3">
                                <button type="button" onClick={() => setIsCreating(false)} className="flex-1 py-2.5 border border-slate-200 rounded-lg font-medium">Cancel</button>
                                <button type="submit" className="flex-1 py-2.5 bg-slate-900 text-white rounded-lg font-bold hover:bg-black">Submit Report</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            {selectedTicket && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in">
                        <div className="p-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                            <div>
                                ...
                                <div className="text-xs text-slate-500">Reported by {selectedTicket.reportedBy} on {new Date(selectedTicket.createdAt).toLocaleDateString()}</div>
                            </div>
                        ...
                        <div className="p-6 overflow-y-auto flex-1 space-y-6">
                            {selectedTicket.status === 'New' && (
                                <div className="bg-purple-50 p-4 rounded-lg border border-purple-100 mb-4">
                                    <h4 className="font-bold text-purple-900 mb-2">New Public Report</h4>
                                    <p className="text-purple-800 text-sm mb-4">This issue was reported by staff. Approve it to create an official ticket and mark asset as 'Under Repair'.</p>
                                    <div className="flex gap-3">
                                        ...
                                        <button onClick={() => handleStatusUpdate('Rejected')} className="px-4 py-2 border border-purple-200 text-purple-700 rounded-lg font-bold hover:bg-purple-100 flex items-center gap-2"><ThumbsDown size={16}/> Reject</button>
                                    </div>
                                </div>
                            )}

                            <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                                <h4 className="font-bold text-slate-800 mb-2 flex items-center gap-2"><AlertCircle size={18} className="text-slate-500"/> Issue Details</h4>
                                <p className="text-slate-700 mb-3">{selectedTicket.description}</p>
                                
                                {selectedTicket.imageBase64 && (
                                    <div className="mb-4">
                                        <div className="text-xs font-bold text-slate-500 uppercase mb-2">Attached Image</div>
                                        <div className="relative group w-fit">
                                            <img src={selectedTicket.imageBase64} alt="Evidence" className="h-48 rounded-lg border border-slate-300 shadow-sm" />
                                ...
                                <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500 border-t border-slate-200 pt-3">
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {['Open', 'In Progress', 'Waiting for Parts'].map(s => (
                                            <button 
                                                ...
                                                className={`px-3 py-1 rounded text-xs font-medium border ${selectedTicket.status === s ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-400'}`}
                                            >
                                                {s}
                                            </button>
                                        ))}
                                    ...
                                    <div className="border-t border-slate-100 pt-4">
                                        <div className="flex items-center justify-between mb-3">
                                            <h4 className="font-bold text-slate-800">Resolution & Closure</h4>
                                            {selectedTicket.assetId && (
                                                <div className="flex bg-slate-100 p-0.5 rounded-lg">
                                                    <button onClick={() => setResolutionType('repair')} className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${resolutionType === 'repair' ? 'bg-white shadow text-slate-900' : 'text-slate-500'}`}>Fix Device</button>
                                                    <button onClick={() => setResolutionType('replace')} className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${resolutionType === 'replace' ? 'bg-white shadow text-slate-900' : 'text-slate-500'}`}>Replace Device</button>
                                                </div>
                                            )}
                                        </div>

                                        {resolutionType === 'replace' ? (
                                            <div className="bg-amber-50 border border-amber-100 rounded-lg p-4 mb-3 animate-in fade-in zoom-in">
                                                <div className="flex items-start gap-3 mb-3">
                                                    <div className="bg-amber-100 p-2 rounded-full text-amber-600"><RefreshCw size={18} /></div>
                                                    <div>
                                                        <h5 className="font-bold text-amber-900 text-sm">Asset Replacement</h5>
                                                ...
                                                <label className="block text-xs font-bold text-amber-800 uppercase mb-1">Select Replacement (from Storage)</label>
                                                <select 
                                                    className="w-full p-2 border border-amber-200 rounded bg-white text-sm mb-3"
                                                ...
                                                <textarea 
                                                    className="w-full p-2 border border-amber-200 rounded text-sm bg-white"
                                                    rows={2}
                                            ...
                                            <>
                                                ...
                                                {selectedTicket.assetId && (
                                                    <label className="flex items-center gap-2 text-sm text-slate-700 mb-4 cursor-pointer">
                                                        <input type="checkbox" checked={autoFixAsset} onChange={e => setAutoFixAsset(e.target.checked)} />
                            ...
                            {selectedTicket.status === 'Resolved' && (
                                <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-lg">
                                    <h4 className="font-bold text-emerald-800 mb-1 flex items-center gap-2"><CheckCircle size={16}/> Resolved</h4>
                                    <p className="text-emerald-700 text-sm">{selectedTicket.resolutionNotes}</p>
                                    <div className="text-xs text-emerald-600 mt-2">Closed on {new Date(selectedTicket.resolvedAt!).toLocaleDateString()}</div>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="flex border-b border-slate-200">
                     <button onClick={() => setActiveTab('inbox')} className={`flex-1 py-3 text-sm font-medium border-b-2 flex items-center justify-center gap-2 ${activeTab === 'inbox' ? 'border-purple-600 text-purple-700 bg-purple-50' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
                        Inbox (New Reports) 
                    ...
                    <button onClick={() => setActiveTab('active')} className={`flex-1 py-3 text-sm font-medium border-b-2 ${activeTab === 'active' ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
                        Active Tickets ({tickets.filter(t => t.status !== 'Resolved' && t.status !== 'New' && t.status !== 'Rejected').length})
                    </button>
                    <button onClick={() => setActiveTab('resolved')} className={`flex-1 py-3 text-sm font-medium border-b-2 ${activeTab === 'resolved' ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
                        History
                    </button>
                </div>
                
                {loading ? <div className="p-8 flex justify-center"><Loader2 className="animate-spin text-slate-400"/></div> : (
                    filteredTickets.length === 0 ? <div className="p-12 text-center text-slate-400">No tickets found.</div> : (
                        <div className="divide-y divide-slate-100">
                            {filteredTickets.map(t => (
                                <div key={t.id} onClick={() => setSelectedTicket(t)} className="p-4 hover:bg-slate-50 cursor-pointer transition-colors flex items-center justify-between group">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                <span className="font-semibold text-slate-800">{t.assetName}</span>
                                                {t.imageBase64 && <ImageIcon size={12} className="text-slate-400"/>}
                                            </div>
                                            <p className="text-sm text-slate-600 line-clamp-1 group-hover:text-slate-900">{t.description}</p>
