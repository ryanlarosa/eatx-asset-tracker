...
const SignHandover: React.FC = () => {
    ...
    // Environment Sync: If link says sandbox, force sandbox mode for this session
    useEffect(() => {
        const urlEnv = searchParams.get('env');
        const currentSandbox = getSandboxStatus();
        if (urlEnv === 'sandbox' && !currentSandbox) {
            localStorage.setItem('eatx_sandbox', 'true');
            window.location.reload();
        } else if (urlEnv !== 'sandbox' && currentSandbox && !urlEnv) {
            // Only switch back if env is explicitly not sandbox in URL but we are in sandbox
            // This is safer than auto-switching back to avoid session loss
        }
    }, [searchParams]);
        ...
        const load = async () => {
            try {
                ...
                if (data && data.status === 'Pending') {
                    ...
                    setError("This link is invalid, expired, or the process has already been completed.");
                }
            } catch (e) {
                setError("Failed to load details. Ensure you are in the correct environment (Sandbox vs Live).");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            alert(`Submission Error: ${e.message || "Please check your network and try again."}`);
            setIsSubmitting(false);
        }
    };

    if (loading) return <div className="min-h-screen w-full bg-slate-50 dark:bg-slate-950 flex items-center justify-center"><Loader2 className="animate-spin text-slate-900 dark:text-white" size={32} /></div>;
    ...
    if (error) return (
        <div className="min-h-screen w-full bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 text-center max-w-md w-full">
                <div className="bg-red-50 dark:bg-red-900/30 p-4 rounded-full text-red-600 dark:text-red-400 w-fit mx-auto mb-4"><AlertTriangle size={40}/></div>
                ...
                {isSandbox && <div className="text-[10px] font-bold text-amber-500 uppercase tracking-widest bg-amber-50 dark:bg-amber-900/20 p-2 rounded">Current: Sandbox Environment</div>}
            </div>
        </div>
    );
    ...
    return (
        <div className="min-h-screen w-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-4 md:p-8 relative">
            ...
            <div className={`w-full max-w-xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-in slide-in-from-bottom-4 duration-500`}>
                ...
                <div className="p-6 md:p-10 flex flex-col items-center">
                    ...
                    <div className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 mb-8">
                        <h3 className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4">{isReturn ? 'Returning Items' : 'Assigned Assets'}</h3>
                        <div className="space-y-4">
                            {pending?.assetsSnapshot.map(a => (
                                <div key={a.id} className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 border-b border-slate-200/50 dark:border-slate-700/50 pb-3 last:border-0 last:pb-0">
                                    <div className="font-bold text-slate-800 dark:text-slate-200 text-sm break-words flex-1 leading-tight">{a.name}</div>
                    ...
                    <div className="w-full mb-8">
                        <div className="flex justify-between items-end mb-3">
                            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider"><PenTool size={14} className="inline mr-1.5 text-slate-400"/> Draw Signature</label>
                            <button onClick={clearCanvas} className="text-xs text-red-600 dark:text-red-400 font-bold hover:underline transition-all">Clear</button>
                        </div>
                        <div ref={containerRef} className="border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl bg-slate-50 dark:bg-slate-200 touch-none relative overflow-hidden h-[200px] shadow-inner w-full">
                            <canvas
                                ref={canvasRef}
