...
const SignHandover: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setError(
          "Failed to load details. Ensure you are in the correct environment (Sandbox vs Live).",
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert(
        `Submission Error: ${e.message || "Please check your network and try again."}`,
      );
    ...
    return (
      <div className="min-h-screen w-full bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 text-center max-w-md w-full">
          ...
          {isSandbox && (
            <div className="text-[10px] font-bold text-amber-500 uppercase tracking-widest bg-amber-50 dark:bg-amber-900/20 p-2 rounded">
      ...
      >
        <div className="bg-slate-900 dark:bg-blue-600 p-8 text-white text-center">
          ...
          <h1 className="text-2xl font-bold tracking-tight">
            {isReturn ? "Asset Return Form" : "EatX IT Hub Handover"}
          </h1>
        ...
        <div className="p-6 md:p-10 flex flex-col items-center">
          <div className="w-full mb-8 text-center md:text-left">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-2">
              Please review the items below. Your signature confirms you have{" "}
              {isReturn
                ? "returned them to IT"
