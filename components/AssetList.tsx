...
const AssetList: React.FC<AssetListProps> = ({
  ...
  const [showImportSuccess, setShowImportSuccess] = useState<number | null>(
    null
  );
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "Lost/Stolen":
        return "bg-gray-800 text-white border-gray-700 dark:bg-gray-700 dark:border-gray-600";
      default:
  ...
  const handlePrintLabel = (asset: Asset) => {
    ...
    const qrHtml = ReactDOMServer.renderToString(
      <QRCode value={deepLinkUrl} size={90} level="M" />
    );
  ...
  const handleDownloadTemplate = async () => {
    ...
    sheet.columns = [
      { header: "System ID", key: "id", width: 30 },
    ...
    try {
      ...
      sheet.eachRow((row: any, rowNumber: number) => {
        ...
        newAssets.push({
          ...
          status: (ASSET_STATUSES.includes(getVal(4) as AssetStatus)
            ? getVal(4)
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col h-full relative">
                ...
                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 z-50 overflow-hidden animate-in fade-in zoom-in-95 origin-top-right">
                  ...
                  <div className="p-2 max-h-60 overflow-y-auto custom-scrollbar">
                    ...
                    {AVAILABLE_COLUMNS.map((col) => (
                      <label
                        key={col.id}
                        className="flex items-center gap-2 p-2 rounded hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer text-sm text-slate-700 dark:text-slate-300"
                      >
                        <input
                          type="checkbox"
            ...
            {canEdit && (
              <>
                <button
                  ...
                  className="p-2 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-2"
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className="p-2 bg-slate-900 dark:bg-blue-600 text-white border border-slate-900 dark:border-blue-600 rounded-lg hover:bg-black dark:hover:bg-blue-700 disabled:opacity-50 flex items-center gap-2"
                    title="Import / Bulk Update"
                  >
                    {isImporting ? (
        ...
        <div className="p-4 bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-3 relative z-20">
          <div className="relative col-span-1">
            <input
              ...
              className="w-full pl-2 pr-2 py-1.5 border border-slate-300 dark:border-slate-700 rounded-md text-sm bg-white dark:bg-slate-900 dark:text-white"
            />
          </div>
          <div className="relative col-span-1">
            <input
          ...
          <div className="col-span-1">
            <select
              ...
              className="w-full py-1.5 px-2 border border-slate-300 dark:border-slate-700 rounded-md text-sm bg-white dark:bg-slate-900 dark:text-white"
            >
          ...
          <button
            ...
            className="col-span-2 xl:col-span-1 py-1.5 px-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-md text-xs font-medium flex items-center justify-center gap-1"
          >
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-950 text-slate-600 dark:text-slate-400 text-xs uppercase tracking-wider font-semibold border-b border-slate-200 dark:border-slate-800">
                <th className="p-4 w-[250px] sticky left-0 bg-slate-50 dark:bg-slate-950 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                  Asset Name
                </th>
                ...
                <th className="p-4 text-right sticky right-0 bg-slate-50 dark:bg-slate-950 z-10 w-[120px] shadow-[-2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                  Actions
                </th>
              </tr>
            </thead>
              ...
              {paginatedAssets.map((asset) => (
                ...
                >
                  <td className="p-4 sticky left-0 bg-white dark:bg-slate-900 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)] group-hover:bg-slate-50 dark:group-hover:bg-slate-800/50">
                    ...
                    {asset.description && (
                      <div className="text-xs text-slate-500 dark:text-slate-500 truncate max-w-[220px] mt-0.5">
                        {asset.description}
                      </div>
                    )}
                  </td>
                  ...
                  {visibleColumns.category && (
                    <td className="p-4">
                      <div className="flex items-center gap-1.5 text-xs font-medium px-2 py-1 bg-slate-100 dark:bg-slate-800 rounded-md w-fit text-slate-700 dark:text-slate-300 whitespace-nowrap">
                  ...
                  {visibleColumns.status && (
                    <td className="p-4">
                      <span
                        className={`px-2.5 py-1 rounded-full text-xs font-bold whitespace-nowrap border ${getStatusColor(
                          asset.status
                        )}`}
                      >
                        {asset.status}
                  ...
                  {visibleColumns.location && (
                    <td className="p-4 text-sm text-slate-600 dark:text-slate-400 flex items-center gap-1">
                  ...
                  {visibleColumns.assignedEmployee && (
                    <td className="p-4">
                      {asset.assignedEmployee ? (
                        <div className="flex items-center gap-1.5 text-xs text-slate-800 dark:text-slate-200 font-medium bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded w-fit whitespace-nowrap">
                  ...
                  {visibleColumns.purchaseCost && (
                    <td className="p-4 text-sm text-slate-700 dark:text-slate-300 text-right font-mono">
                      {asset.purchaseCost
                  ...
                  <td className="p-4 text-right sticky right-0 bg-white dark:bg-slate-900 z-10 group-hover:bg-slate-50 dark:group-hover:bg-slate-800/50 shadow-[-2px_0_5px_-2px_rgba(0,0,0,0.05)]">
                    <div className="flex justify-end gap-1">
                      <button
                        ...
                        className="p-1.5 text-slate-500 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-slate-700 rounded-md"
                      ...
                      {canEdit ? (
                        <>
                          <button
                            ...
                            className="p-1.5 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-md"
                            title="Edit"
                          >
                            <Edit2 size={16} />
                          </button>
                      ...
                      {!canEdit && !canDelete && (
                        <span className="p-2 text-slate-300 dark:text-slate-600 cursor-not-allowed">
                          <Lock size={16} />
                        </span>
                      )}
                    </div>
        ...
        <div className="md:hidden bg-slate-50 dark:bg-slate-950 p-4 space-y-4 min-h-[400px] pb-40">
          {paginatedAssets.map((asset) => (
            <div
              key={asset.id}
              className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col gap-3"
            >
              <div className="flex justify-between items-start">
                ...
                <span
                  className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide border whitespace-nowrap ${getStatusColor(
                    asset.status
                  )}`}
                >
                  {asset.status}
              ...
              <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-sm text-slate-600 dark:text-slate-400 border-t border-slate-100 dark:border-slate-800 pt-3">
                <div>
                  ...
                  <div className="flex items-center gap-1 truncate">
                    <Tag size={12} className="shrink-0" /> {asset.category}
                  </div>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 uppercase font-bold block mb-0.5">
                  ...
                  <div className="flex items-center gap-1 truncate">
                    <MapPin size={12} className="shrink-0" /> {asset.location}
                  </div>
                </div>
              ...
              <div className="flex gap-2 border-t border-slate-100 dark:border-slate-800 pt-3 mt-1">
                <button
                  ...
                  className="p-2 text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-lg"
                >
                  <QrCode size={16} />
                </button>
                {canEdit && (
                  <>
                    <button
                      ...
                      className="flex-1 py-2 bg-slate-900 dark:bg-blue-600 text-white rounded-lg text-xs font-bold flex items-center justify-center gap-2 hover:bg-black dark:hover:bg-blue-700 shadow-sm"
                    >
                      <Edit2 size={14} /> Edit Asset
                    </button>
                    <button
                      onClick={() => onDuplicate(asset)}
                      className="p-2 text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 border border-blue-100 dark:border-blue-900/50 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/50"
                    >
                      <Copy size={16} />
                    </button>
                  </>
                ...
                {canDelete && (
                  <button
                    ...
                    className="p-2 text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 border border-red-100 dark:border-red-900/50 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/50"
                  >
                    <Trash2 size={16} />
                  </button>
                )}
