...
import { 
  ...
  Printer, Clock, Trash2, ExternalLink, Loader2 
} from 'lucide-react';
...
const StaffView: React.FC = () => {
  ...
  const handlePrintDocument = (doc: HandoverDocument) => {
      ...
      const docTitle = doc.type === 'Return' ? 'ASSET RETURN FORM' 
      ...
      if (doc.type === 'Return') {
          ...
          signatureBlock = `
            <div class="sig-section">
                <div class="sig-box">
                    ${empSig}
                    <div class="sig-label"><strong>Acknowledged by:</strong> ${doc.employeeName}</div>
        ...
        <body>
            ...
            <div class="doc-identity">
                ...
                <table class="meta-table">
                    <tr>
                        ...
                        <td style="color: #000; font-weight: 600;">${doc.employeeName}</td>
                        <td width="100" align="right"><strong>Date:</strong></td>
                        <td width="140" align="right">${dateStr}</td>
                    </tr>
                    <tr>
                        <td><strong>Document ID:</strong></td>
                        <td style="font-family: monospace; font-size: 9pt;">${doc.id}</td>
            ...
            <table class="items-table">
                ...
                <tbody>
                    ${doc.assets.map((a, i) => `
                        <tr>
                            <td align="center" style="color: #94a3b8;">${i + 1}</td>
                            <td style="font-weight: 600; color: #0f172a;">${a.name}</td>
                            <td style="font-family: monospace; font-size: 10pt; color: #334155;">${a.serialNumber || 'N/A'}</td>
                        </tr>
                    `).join('')}
                </tbody>
            </table>
    ...
    <div className="space-y-6 relative">
      <header className="flex flex-col md:flex-row justify-between md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">Assign assets or offboard staff equipment.</p>
        </div>
        <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-lg self-start">
            ...
            <button onClick={() => setView('pending')} className={`px-4 py-2 text-sm font-medium rounded-md flex items-center gap-2 ${view === 'pending' ? 'bg-white dark:bg-slate-700 shadow text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
      ...
      {view === 'manage' ? (
        <>
            ...
            {mode === 'offboard' && selectedEmployee ? (
                ...
                <div className="relative mb-6">
                    ...
                    <input 
                    type="text" 
                    placeholder={mode === 'onboard' ? "Filter available assets..." : "Search Employee Name..."}
                    value={searchTerm}
            ...
            <div className="border-t border-slate-100 dark:border-slate-800 pt-6">
                ...
                <div className="space-y-3">
                    <div>
                    <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1 uppercase">Assign To (User Name)</label>
                    <input 
                    ...
                    <div className="relative flex py-1 items-center">
                        ...
                        <span className="flex-shrink mx-2 text-slate-400 text-[10px] uppercase font-bold">OR</span>
                        <div className="flex-grow border-t border-slate-200 dark:border-slate-700"></div>
                    </div>
                    <button 
                        onClick={handleHandoverClick}
                        disabled={selectedAssetIds.size === 0 || !targetEmployee || isProcessing}
                        className="w-full bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 py-2 rounded-lg font-medium hover:bg-slate-200 dark:hover:bg-slate-600 border border-slate-200 dark:border-slate-700"
                    >
                        Sign & Assign Now
                    </button>
                </div>
                ...
                <div className="space-y-3">
                    <div className={`transition-opacity ${!selectedEmployee ? 'opacity-50 pointer-events-none' : ''}`}>
                        ...
                        >
                            {isProcessing ? 'Generating...' : <>Generate Return Link <LinkIcon size={16}/></>}
                        </button>
                        
                        <div className="relative flex py-2 items-center">
                            <div className="flex-grow border-t border-slate-200 dark:border-slate-700"></div>
                            <span className="flex-shrink mx-2 text-slate-400 text-[10px] uppercase font-bold">OR Direct Sign</span>
                            <div className="flex-grow border-t border-slate-200 dark:border-slate-700"></div>
                        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {pendingLinks.length > 0 ? pendingLinks.map(link => (
                <div key={link.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden hover:shadow-md transition-all">
                    <div className="p-4 border-b bg-slate-50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-800 flex justify-between items-start">
                        <div>
                            <div className="font-bold text-slate-800 dark:text-white">{link.employeeName}</div>
                            <div className="text-[10px] text-slate-400 uppercase font-bold tracking-tight">Pending {link.type || 'Signature'}</div>
                        </div>
                        <button 
                    ...
                    <div className="p-4">
                        <div className="space-y-1 mb-4">
                            <div className="text-xs text-slate-400 font-bold mb-1">Items to {link.type === 'Return' ? 'Return' : 'Assign'}:</div>
                            {link.assetsSnapshot.map(a => (
                                <div key={a.id} className="text-xs text-slate-600 dark:text-slate-400 flex justify-between">
                                    <span className="truncate flex-1">{a.name}</span>
                                    <span className="font-mono opacity-60 ml-2">{a.serialNumber}</span>
                                </div>
                            ))}
                        </div>
                        <div className="border-t border-slate-50 dark:border-slate-800 pt-3 flex items-center justify-between text-[10px] text-slate-400">
                             <div className="flex items-center gap-1"><Clock size={10}/> Sent {new Date(link.createdAt).toLocaleDateString()}</div>
                             <div className="flex items-center gap-2">
                                <button 
                                    onClick={() => {
                                        const isSandbox = getSandboxStatus();
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {documents.length > 0 ? documents.map(doc => {
                // Only Flag Return as 'Action Required' if it's pending IT verification
                const isPendingReturn = doc.type === 'Return' && doc.status !== 'Completed';
                const isCompleted = doc.status === 'Completed';
                
                return (
                <div key={doc.id} className={`bg-white dark:bg-slate-900 rounded-xl border shadow-sm overflow-hidden hover:shadow-md transition-all ${isPendingReturn ? 'border-amber-200 dark:border-amber-800 shadow-amber-50 dark:shadow-none' : 'border-slate-200 dark:border-slate-800'}`}>
                    <div className={`p-4 border-b flex justify-between items-start ${isPendingReturn ? 'bg-amber-50/50 dark:bg-amber-900/10 border-amber-100 dark:border-amber-800' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-800'}`}>
                        <div>
                            ...
                            <div className="text-xs text-slate-500 dark:text-slate-400">{new Date(doc.date).toLocaleDateString()}</div>
                        </div>
                        <div className="flex flex-col items-end gap-1">
                             ...
                             {isCompleted && <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-1.5 rounded">Completed</span>}
                        </div>
                    </div>
                    <div className="p-4">
                        ...
                        <div className="space-y-1 mb-4">
                            {doc.assets.slice(0, 3).map(a => (
                                <div key={a.id} className="text-sm text-slate-700 dark:text-slate-300 flex justify-between">
                                    <span className="truncate flex-1">{a.name}</span>
                                    <span className="text-slate-400 text-xs font-mono ml-2">{a.serialNumber}</span>
                                </div>
                            ))}
                            ...
                            <div className="flex items-end justify-between">
                                <div>
                                    ...
                                    <div className="flex gap-2">
                                        <div className="bg-white p-1 rounded border border-slate-100 dark:border-slate-800 w-fit" title="Employee Signature">
                                        ...
                                        {doc.type === 'Return' && (
                                          <div className="bg-white p-1 rounded border border-slate-100 dark:border-slate-800 w-fit" title="IT Verified Signature">
                                ...
                                <div className="flex gap-2">
                                    <button onClick={() => handlePrintDocument(doc)} className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white bg-slate-50 dark:bg-slate-800 rounded-lg" title="Print Document"><Printer size={16}/></button>
                                    {isPendingReturn && (
                                        <button 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl max-w-md w-full p-6 border border-slate-100 dark:border-slate-800 animate-in fade-in zoom-in">
                ...
                <p className="text-slate-600 dark:text-slate-400 text-sm mb-4">
                    A digital form has been created for <span className="font-bold text-slate-900 dark:text-white">{linkModal.name}</span>. Send this link to the employee to collect their signature.
                </p>
