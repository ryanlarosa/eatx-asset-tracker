...
const StaffView: React.FC = () => {
  ...
  const filteredDocuments = useMemo(() => {
    return documents.filter((doc) => {
      const matchesSearch =
        doc.employeeName.toLowerCase().includes(docSearchTerm.toLowerCase()) ||
        doc.assets.some(
          (a) =>
            a.name.toLowerCase().includes(docSearchTerm.toLowerCase()) ||
            a.serialNumber.toLowerCase().includes(docSearchTerm.toLowerCase())
        );
  ...
  if (!canEdit) {
    return (
      <div className="p-8 text-center text-slate-500 dark:text-slate-400">
        You do not have permission to manage staff assets in IT Hub.
      </div>
    );
  }
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row gap-4 items-center">
            <div className="relative flex-1 w-full">
              ...
              />
              <input
                className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm dark:text-white"
                placeholder="Search employee, asset or serial..."
                value={docSearchTerm}
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
              ...
              <select
                className="flex-1 md:w-36 p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-bold text-slate-600 dark:text-slate-300"
                value={docTypeFilter}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                  <tr>
                    <th className="p-4 font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-[10px]">
                      Date & ID
                    </th>
                    <th className="p-4 font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-[10px]">
                      Employee
                    ...
                    <th className="p-4 font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-[10px] text-right">
                      Actions
                    </th>
                  </tr>
                </thead>
                          ...
                          <td className="p-4">
                            <div className="text-xs text-slate-600 dark:text-slate-400">
                              <span className="font-bold">
                                {doc.assets.length} items
                              </span>
                              <div className="text-[10px] truncate max-w-[150px] opacity-70 mt-0.5">
                                {doc.assets.map((a) => a.name).join(", ")}
                              </div>
                            </div>
                          </td>
                          <td className="p-4">
                            {isPendingAction ? (
                              <div className="flex items-center gap-1.5 text-amber-600 dark:text-amber-400 text-xs font-bold animate-pulse">
                                <AlertTriangle size={14} /> IT verification
                              </div>
                            ) : (
                            ...
                            <div className="flex justify-end gap-2">
                              ...
                              {isPendingAction ? (
                                <button
                                  ...
                                  className="px-3 py-1 bg-amber-500 text-white rounded-lg text-xs font-bold hover:bg-amber-600 shadow-sm flex items-center gap-1"
                                >
                                  Verify Now <ChevronRight size={14} />
                                </button>
                              ) : (
                                <div className="p-1.5 text-slate-200 dark:text-slate-700">
