...
import { ShoppingBag, Clock, CheckCircle, XCircle, ChevronRight, User, AlertCircle, Link as LinkIcon, Box, Loader2, ArrowRight, DollarSign, Send, Save, Search, Building2 } from 'lucide-react';
                                    ...
                                    {filteredRequests.map(r => (
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 mb-2">
                                                {r.reason.toLowerCase().includes('for department') ? <Building2 size={12}/> : <User size={12}/>} 
                                ...
                                <div className="h-full flex flex-col">
                                    ...
                                    <div className="p-6 flex-1">
                                        <div className="bg-white dark:bg-slate-900 p-4 rounded-lg border border-slate-200 dark:border-slate-800 mb-6 shadow-sm">
                                            <h4 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2">Request Details</h4>
                                        ...
                                        <div className="bg-slate-50 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
                                            <h3 className="font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                                                Action Hub: <span className="text-slate-500 dark:text-slate-400 font-normal">{selectedRequest.status}</span>
                                            </h3>

                                            {selectedRequest.status === 'New' && (
                                                <div className="space-y-3">
                                                    <p className="text-sm text-slate-600 dark:text-slate-400">Review procurement details. Acknowledge to start sourcing or sourcing from stock.</p>
                                                    <div className="flex gap-3">
                                                        <button onClick={() => updateStatus('Acknowledged', 'Request acknowledged.')} disabled={isProcessing} className="flex-1 bg-slate-900 dark:bg-blue-600 text-white py-2 rounded-lg font-bold hover:bg-black dark:hover:bg-blue-700 flex justify-center items-center gap-2">
                                                            {isProcessing ? <Loader2 className="animate-spin"/> : <CheckCircle size={16}/>} Acknowledge
                                                        </button>
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                        <div className="bg-white dark:bg-slate-900 p-4 rounded-lg border border-slate-200 dark:border-slate-800">
                                                            ...
                                                            <div className="relative mb-2">
                                                                <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 mb-1 uppercase">Recipient Name (User or Area)</label>
                                                                <input 
                                                                ...
                                                                />
                                                                {showUserDropdown && deployToUser && filteredEmployees.length > 0 && (
                                                        ...
                                                        <div className="bg-white dark:bg-slate-900 p-4 rounded-lg border border-slate-200 dark:border-slate-700 flex flex-col justify-center text-center">
                                                            ...
                                                            <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">Escalate to Finance for approval and purchase.</p>
                                                            <button 
                                                                onClick={() => updateStatus('Pending Finance', 'No stock. Escaped to Finance.')}
                                            ...
                                            {selectedRequest.status === 'Approved' && (
                                                <div className="space-y-3">
                                                    <div className="bg-indigo-50 dark:bg-indigo-900/30 p-3 rounded text-indigo-800 dark:text-indigo-300 text-sm mb-2 border border-indigo-100 dark:border-indigo-900/50">
                                                        <span className="font-bold">Ready to Deploy.</span> Source the asset, register it in the inventory, then assign it below.
                                                    </div>
                                                    
                                                    <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Select Received Asset</label>
                                                    <select className="w-full p-2 border border-slate-300 dark:border-slate-700 rounded text-sm mb-2 bg-white dark:bg-slate-950 dark:text-white" value={selectedAssetId} onChange={e => setSelectedAssetId(e.target.value)}>
                                                        <option value="">Select Asset to Deploy...</option>
                                                        {inventoryAssets.map(a => <option key={a.id} value={a.id}>{a.name} ({a.serialNumber})</option>)}
                                                    </select>

                                                    <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Assign To Recipient</label>
                                                    <div className="relative mb-2">
                                                        <input 
                                                            className="w-full p-2 border border-slate-300 dark:border-slate-700 rounded text-sm bg-white dark:bg-slate-950 dark:text-white"
                                                            placeholder="Employee Name or Dept Area..."
                                                    ...
                                                    <div className="relative">
                                                        <textarea 
                                                            ...
                                                            onChange={e => setNotes(e.target.value)}
                                                        />
                                                        <button onClick={handleSaveProgress} className="absolute top-2 right-2 text-slate-400 hover:text-slate-600 p-1"><Save size={16} /></button>
                                                    </div>
                                                    <button 
                                                        onClick={handleDeploy}
                                                        disabled={!selectedAssetId || !deployToUser || isProcessing}
                                            ...
                                            {(selectedRequest.status === 'Deployed' || selectedRequest.status === 'Rejected') && (
                                                <div className={`p-4 rounded-lg border ${selectedRequest.status === 'Deployed' ? 'bg-emerald-50 dark:bg-emerald-900/30 border-emerald-100 dark:border-emerald-900/50' : 'bg-red-50 dark:bg-red-900/30 border-red-100 dark:border-red-900/50'}`}>
                                                    <div className={`font-bold flex items-center gap-2 mb-1 ${selectedRequest.status === 'Deployed' ? 'text-emerald-800 dark:text-emerald-400' : 'text-red-800 dark:text-red-400'}`}>
                                                        ...
                                                        {selectedRequest.status === 'Deployed' ? 'Procurement Completed' : 'Request Rejected'}
                                                    </div>
