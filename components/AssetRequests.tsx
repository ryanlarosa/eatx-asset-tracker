...
const AssetRequests: React.FC = () => {
  ...
  // Smart logic for beneficiary extraction
  useEffect(() => {
    if (selectedRequest) {
      ...
      // Priority 1: Check for Employee pattern
      const empMatch = reason.match(/For Employee:\s*([^\n]+)/i);
      if (empMatch && empMatch[1]) {
        setDeployToUser(empMatch[1].trim());
      }
      // Priority 2: Check for Department pattern
      else {
        const deptMatch = reason.match(/For Department:\s*([^\n]+)/i);
        if (deptMatch && deptMatch[1]) {
          // Use "Department: [Name]" as the beneficiary string
          setDeployToUser(deptMatch[1].trim());
        } else {
          setDeployToUser("");
        }
      }
  ...
  const filteredEmployees = uniqueEmployees.filter((e) =>
    e.toLowerCase().includes(deployToUser.toLowerCase())
  );
      ...
      <header className="flex flex-col md:flex-row justify-between md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
            <ShoppingBag className="text-slate-900 dark:text-white" /> Asset
            Requests
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            Track lifecycle: Request → Acknowledge → Finance/Stock → Deploy
          </p>
        </div>
        <button
          onClick={() => navigator.clipboard.writeText(publicLink)}
          className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 px-3 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-2 text-sm font-medium w-fit"
        >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden min-h-[600px] flex flex-col">
        <div className="flex border-b border-slate-200 dark:border-slate-800">
          ...
          >
            Active Queue
            {requests.filter((r) =>
              ["New", "Acknowledged", "Pending Finance", "Approved"].includes(
                r.status
              )
            ).length > 0 && (
          ...
          <button
            ...
            className={`flex-1 py-3 text-sm font-medium border-b-2 ${
              activeTab === "history"
                ? "border-slate-900 dark:border-blue-600 text-slate-900 dark:text-white"
                : "border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"
            }`}
          >
        ...
        {loading ? (
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div
              className={`${
                selectedRequest ? "hidden md:block w-1/3" : "w-full"
              } border-r border-slate-200 dark:border-slate-800 overflow-y-auto bg-slate-50/50 dark:bg-slate-950/50`}
            >
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                  {filteredRequests.map((r) => (
                    <div
                      ...
                      className={`p-4 cursor-pointer hover:bg-white dark:hover:bg-slate-900 transition-colors ${
                        selectedRequest?.id === r.id
                          ? "bg-white dark:bg-slate-900 border-l-4 border-l-slate-900 dark:border-l-blue-600 shadow-sm"
                    ...
                    >
                      <div className="flex justify-between items-start mb-1">
                        ...
                        <span
                          className={`text-[10px] px-1.5 py-0.5 rounded border uppercase font-bold ${getUrgencyColor(
                            r.urgency
                          )}`}
                        >
                          {r.urgency}
                      ...
                      <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 mb-2">
                        {r.reason.toLowerCase().includes("for department") ? (
                          <Building2 size={12} />
                        ) : (
                          <User size={12} />
                        )}
            ...
            <div
              className={`${
                selectedRequest ? "w-full md:w-2/3" : "hidden"
                ...
                <div className="h-full flex flex-col">
                  <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-start bg-white dark:bg-slate-900">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <h2 className="text-xl font-bold text-slate-900 dark:text-white">
                          {selectedRequest.category} Request
                        </h2>
                    ...
                    <button
                      ...
                      className="md:hidden p-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm"
                    >
                      Back
                    </button>
                  </div>
                    ...
                    <div className="px-6 py-4 bg-slate-50/50 dark:bg-slate-950/30 border-b border-slate-100 dark:border-slate-800">
                      <div className="flex items-center text-xs font-medium text-slate-500 dark:text-slate-400">
                        {[
                          ...
                          { l: "Finance/Stock", s: "Pending Finance" },
                          { l: "Deploy", s: "Deployed" },
                        ].map((step, idx) => {
                          const currentStepIdx = getStatusStep(
                      ...
                      {selectedRequest.status === "New" && (
                        <div className="space-y-3">
                          <p className="text-sm text-slate-600 dark:text-slate-400">
                            Review procurement details. Acknowledge to start
                            sourcing or sourcing from stock.
                          </p>
                          <div className="flex gap-3">
                            <button
                              ...
                              className="flex-1 bg-slate-900 dark:bg-blue-600 text-white py-2 rounded-lg font-bold hover:bg-black dark:hover:bg-blue-700 flex justify-center items-center gap-2"
                            >
                              {isProcessing ? (
                            ...
                            <button
                              onClick={() =>
                                updateStatus("Rejected", "Rejected by IT.")
                              }
                              disabled={isProcessing}
                              className="px-4 py-2 border border-red-200 dark:border-red-900/50 text-red-600 dark:text-red-400 rounded-lg font-bold hover:bg-red-50 dark:hover:bg-red-900/20"
                            >
                              Reject
                            </button>
                          </div>
                        ...
                        <div className="space-y-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="bg-white dark:bg-slate-900 p-4 rounded-lg border border-slate-200 dark:border-slate-800">
                              ...
                              <div className="relative mb-2">
                                ...
                                <input
                                  ...
                                  placeholder="Employee Name or Dept Area..."
                                  value={deployToUser}
                                  onChange={(e) => {
                                    setDeployToUser(e.target.value);
                                  ...
                                  filteredEmployees.length > 0 && (
                                    <div className="absolute left-0 right-0 top-full mt-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg z-10 max-h-40 overflow-y-auto">
                                      {filteredEmployees.map((emp) => (
                                        <div
                                          key={emp}
                                          className="p-2 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer text-sm"
                                          onClick={() => setDeployToUser(emp)}
                                        >
                                          {emp}
                                        </div>
                                      ))}
                            ...
                            <div className="bg-white dark:bg-slate-900 p-4 rounded-lg border border-slate-200 dark:border-slate-700 flex flex-col justify-center text-center">
                              <div className="font-bold text-slate-800 dark:text-white mb-2 flex items-center justify-center gap-2">
                                <DollarSign size={16} /> No Stock?
                              </div>
                              <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">
                                Escalate to Finance for approval and purchase.
                              </p>
                              <button
                                onClick={() =>
                                  updateStatus(
                                    "Pending Finance",
                                    "No stock. Escaped to Finance."
                                  )
                                }
                                disabled={isProcessing}
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="relative">
                            <textarea
                              ...
                              placeholder="Approver name / Budget code..."
                              value={notes}
                          ...
                          <div className="flex gap-3">
                            <button
                              onClick={() =>
                                updateStatus("Approved", "Finance Approved.")
                              }
                              disabled={isProcessing}
                            ...
                            <button
                              onClick={() =>
                                updateStatus("Rejected", "Finance Rejected.")
                              }
                              disabled={isProcessing}
                              className="px-4 py-2 border border-red-200 dark:border-red-900/50 text-red-600 dark:text-red-400 rounded-lg font-bold hover:bg-red-50 dark:hover:bg-red-900/20"
                            >
                      ...
                      {selectedRequest.status === "Approved" && (
                        <div className="space-y-3">
                          <div className="bg-indigo-50 dark:bg-indigo-900/30 p-3 rounded text-indigo-800 dark:text-indigo-300 text-sm mb-2 border border-indigo-100 dark:border-indigo-900/50">
                            ...
                            Source the asset, register it in the inventory, then
                            assign it below.
                          </div>

                          <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">
                            Select Received Asset
                          </label>
                          <select
                            className="w-full p-2 border border-slate-300 dark:border-slate-700 rounded text-sm mb-2 bg-white dark:bg-slate-950 dark:text-white"
                            value={selectedAssetId}
                          ...
                          <button
                            onClick={handleDeploy}
                            disabled={
                              !selectedAssetId || !deployToUser || isProcessing
                            }
                      ...
                      {(selectedRequest.status === "Deployed" ||
                        ...
                        <div
                          className={`p-4 rounded-lg border ${
                            selectedRequest.status === "Deployed"
                              ? "bg-emerald-50 dark:bg-emerald-900/30 border-emerald-100 dark:border-emerald-900/50"
                        ...
                        >
                          ...
                          {selectedRequest.linkedAssetId && (
                            <div className="mt-3 pt-3 border-t border-emerald-200/50 dark:border-emerald-800/50 flex items-center gap-2 text-sm">
                              ...
                              <span className="font-mono bg-white dark:bg-slate-900 px-2 py-0.5 rounded border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-400 font-bold">
                                {
                                  assets.find(
                                    (a) =>
                                      a.id === selectedRequest.linkedAssetId
                                  )?.name
                                }
                              </span>
                            </div>
