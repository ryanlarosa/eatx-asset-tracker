...
const PublicAssetRequest: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        "Submission failed. Your data might be partially saved. Please check with IT before retrying."
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const inputClass =
    "w-full p-3 border border-slate-300 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-950 text-slate-900 dark:text-white focus:ring-2 focus:ring-slate-900 dark:focus:ring-blue-600 transition-all shadow-sm";
  const labelClass =
    "block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase mb-1.5 tracking-widest";
    ...
    return (
      <div className="min-h-screen w-full bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-slate-900 p-10 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 text-center max-w-md w-full animate-in fade-in zoom-in duration-300">
          <div className="bg-emerald-50 dark:bg-emerald-900/30 p-5 rounded-full text-emerald-600 dark:text-emerald-400 w-fit mx-auto mb-6 shadow-sm border border-emerald-100 dark:border-emerald-800">
            <CheckCircle size={48} />
          </div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mb-8 leading-relaxed">
            ...
            <strong>
              {requestItems.filter((i) => i.itemName.trim()).length}
            </strong>{" "}
            item(s). The IT Hub team has been notified and will reach out to{" "}
            <strong>{submitterEmail}</strong>.
          </p>
          <div className="flex flex-col gap-3">
            <button
              onClick={() => window.location.reload()}
              className="w-full py-3 bg-slate-900 dark:bg-blue-600 text-white rounded-2xl font-bold hover:bg-black dark:hover:bg-blue-700 shadow-xl shadow-slate-900/10"
            >
  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 dark:bg-slate-950 flex flex-col items-center p-4 md:p-8 relative">
      ...
      <div
        className={`max-w-xl w-full bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden my-8 ${
          isSandbox ? "mt-10" : ""
        }`}
      >
        <div className="bg-slate-900 dark:bg-blue-600 p-8 text-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-300 dark:text-blue-100 text-sm font-medium opacity-80 uppercase tracking-widest text-[10px] mt-1">
                Unified Asset Request Portal
              </p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-10 space-y-8">
          ...
          <div className="space-y-6 bg-slate-50/50 dark:bg-slate-800/30 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800">
              ...
              <div className="grid grid-cols-2 gap-3 p-1.5 bg-white dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-inner">
                <button
                  ...
                  className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                    requestType === "individual"
                      ? "bg-slate-900 dark:bg-blue-600 shadow-lg text-white"
            ...
            {requestType === "individual" && (
              <div className="animate-in slide-in-from-top-2 duration-300">
                ...
                <div className="relative">
                  ...
                  <input
                    required
                    placeholder="Staff member receiving asset"
                    className={`pl-11 ${inputClass}`}
                    value={employeeName}
            ...
            {requestType === "department" && (
              <div className="flex items-center gap-3 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-2xl text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase tracking-widest leading-relaxed border border-blue-100 dark:border-blue-900/30">
                ...
                <span>
                  The asset will be linked to the general outlet/office area
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center px-1">
              ...
              <button
                type="button"
                onClick={addItem}
                className="text-[10px] flex items-center gap-1 text-blue-600 dark:text-blue-400 font-black uppercase tracking-widest hover:bg-blue-50 dark:hover:bg-blue-900/20 px-3 py-1.5 rounded-full transition-all border border-blue-100 dark:border-blue-900/30"
              >
            ...
            <div className="space-y-3">
              {requestItems.map((item, index) => (
                <div
                  key={item.id}
                  className="flex gap-3 items-center animate-in slide-in-from-left-2 fade-in group"
                >
                  <div className="flex-1 relative">
                    <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-300 dark:text-slate-700">
                      {index + 1}
                    </div>
                    <input
                      required
          ...
          <div>
            ...
            <textarea
              required
              rows={3}
              placeholder="Why is this needed? (e.g. New joiner, equipment failure, outlet expansion)"
              className={inputClass}
              value={reason}
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full bg-emerald-600 text-white py-5 rounded-3xl font-black uppercase tracking-widest text-xs hover:bg-emerald-700 disabled:opacity-50 flex justify-center items-center gap-3 shadow-2xl shadow-emerald-900/20 transition-all active:scale-95 group"
          >
            {submitting ? (
      ...
      <p className="mt-8 text-slate-400 text-[10px] font-bold uppercase tracking-[0.2em] opacity-40">
        System Powered by EatX IT Hub
      </p>
    </div>
  );
};
