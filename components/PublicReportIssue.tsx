    ...
    if (submitted) return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
            <div className="bg-white p-8 rounded-xl shadow-lg border border-slate-200 text-center max-w-md w-full animate-in fade-in zoom-in">
                ...
                <p className="text-slate-500 mb-6">Thank you. The IT team has been notified and will review your issue shortly.</p>
                <button onClick={() => window.location.reload()} className="text-sm text-slate-600 hover:text-slate-900 underline">Submit another report</button>
            </div>
        </div>
    );
        ...
        <div className="min-h-screen bg-slate-50 py-12 px-4">
            <div className="max-w-lg mx-auto bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-5">
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Your Name / Role</label>
                        <input required placeholder="e.g. John (Bar Manager)" className="w-full p-3 border border-slate-300 rounded-lg" value={reportedBy} onChange={e => setReportedBy(e.target.value)} />
                    </div>

                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Location</label>
                        <select required className="w-full p-3 border border-slate-300 rounded-lg bg-white" value={location} onChange={e => { setLocation(e.target.value); handleAssetChange(''); }}>
                    ...
                    {location && (
                        <div className="animate-in slide-in-from-top-2 space-y-4">
                             <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Select Device (Optional)</label>
                                <select className="w-full p-3 border border-slate-300 rounded-lg bg-white" value={assetId} onChange={e => handleAssetChange(e.target.value)}>
                             ...
                             <div className={`space-y-4 p-4 bg-slate-50 rounded-lg border border-slate-100 ${assetId ? 'opacity-50 pointer-events-none' : ''}`}>
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Device Name / Description</label>
                                    <input required={!assetId} placeholder="e.g. Kitchen Printer 2" className="w-full p-2 border border-slate-300 rounded-lg text-sm" value={assetName} onChange={e => setAssetName(e.target.value)} disabled={!!assetId}/>
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Device Type</label>
                                        <select required={!assetId} className="w-full p-2 border border-slate-300 rounded-lg text-sm bg-white" value={deviceType} onChange={e => setDeviceType(e.target.value)} disabled={!!assetId}>
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Serial No. (Optional)</label>
                                        <input placeholder="e.g. SN123456" className="w-full p-2 border border-slate-300 rounded-lg text-sm" value={reportedSerial} onChange={e => setReportedSerial(e.target.value)} disabled={!!assetId}/>
                                    </div>
                                </div>
                             </div>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea required rows={3} placeholder="Describe the issue..." className="w-full p-3 border border-slate-300 rounded-lg" value={description} onChange={e => setDescription(e.target.value)} />
                    </div>

                    {/* Image Upload */}
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Upload Photo (Clarification)</label>
                    ...
                    <div>
                        ...
                        <div className="flex gap-2">
                            {['Low', 'Medium', 'High', 'Critical'].map(p => (
                                <button type="button" key={p} onClick={() => setPriority(p)} className={`flex-1 py-2 rounded-lg text-sm font-medium border ${priority === p ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-600 border-slate-200'}`}>
                                    {p}
                                </button>
                            ))}
                        </div>
