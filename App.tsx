...
const App = () => {
  ...
  const [isDarkMode, setIsDarkMode] = useState(
    () => localStorage.getItem("eatx_theme") === "dark"
  );
  ...
  useEffect(() => {
    if (!user) return;
    setLoading(true);
    const unsubAssets = listenToAssets((data) => {
      setAssets(data);
      setLoading(false);
    });
    const unsubProjects = listenToProjects(async (data) => {
      setProjects(data);
      setNotificationCount((await getOverdueItems(data)).length);
    });
  ...
  return (
    <Router>
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans transition-colors duration-200 overflow-x-hidden">
        <Routes>
          {/* Public Routes are top-level and centered */}
