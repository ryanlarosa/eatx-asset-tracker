...
export const completePendingHandover = async (id: string, signature: string) => {
  ...
  // 1. Save the signed document
  batch.set(doc(db, getColName("documents"), docId), sanitizeData(handoverDoc));
  
  // 2. Mark the pending link as completed
  batch.update(doc(db, getColName("pendingHandovers"), id), { status: "Completed" });
  
  // 3. Update Asset Registry immediately
  if (assetIds.length > 0) {
    if (data.type === "Handover") {
      bulkAssignAssetsInternal(batch, assetIds, data.employeeName, docId, "Staff Signed");
    } else if (data.type === "Transfer" && data.targetName) {
      bulkTransferAssetsInternal(batch, assetIds, data.targetName, docId, "Staff Signed");
    } else if (data.type === "Return") {
      bulkReturnAssetsInternal(batch, assetIds, docId, "Staff Signed");
    }
  }
  
  // Execute transaction. If this fails, it's likely due to Firestore Rules blocking public writes to 'assets' or 'logs'.
  await batch.commit();
