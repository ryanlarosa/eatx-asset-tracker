...
import { 
  ...
  onSnapshot, query, where, orderBy, limit, writeBatch, QuerySnapshot, DocumentData
} from 'firebase/firestore';
import { 
  getAuth, signInWithEmailAndPassword, signOut, onAuthStateChanged, createUserWithEmailAndPassword 
} from 'firebase/auth';
import { 
  Asset, AssetLog, AppConfig, UserProfile, UserRole, Project, 
  AssetStats, HandoverDocument, PendingHandover, IncidentReport, 
  AssetRequest, Invoice, ProjectItem, AppNotification, EmailConfig, Task
} from '../types';
...
export const checkEnvStatus = () => {
    ...
    if (activeConfig.apiKey && activeConfig.apiKey !== "PASTE_YOUR_API_KEY_HERE") return { ok: true, message: message + " (Fallback)" };
...
export const sendSystemEmail = async (subject: string, message: string, link: string = '', specificRecipient?: string) => {
...
export const listenToNotifications = (cb: (notifications: AppNotification[]) => void) => {
    const q = query(collection(db, getColName('notifications')), orderBy('timestamp', 'desc'), limit(20));
    return onSnapshot(q, (snapshot) => cb(snapToData<AppNotification>(snapshot)));
};

export const createNotification = async (type: AppNotification['type'], title: string, message: string, link?: string) => {
    try {
        const id = 'notif-' + Date.now() + Math.random().toString(36).substr(2, 5);
        await setDoc(doc(db, getColName('notifications'), id), {
            id, type, title, message, timestamp: new Date().toISOString(), read: false, link
        });
...
export const markAllNotificationsRead = async () => {
    const q = query(collection(db, getColName('notifications')), where('read', '==', false));
...
export const listenToTasks = (cb: (tasks: Task[]) => void) => {
    return onSnapshot(query(collection(db, getColName('tasks')), orderBy('createdAt', 'desc')), (s) => cb(snapToData<Task>(s)));
};

export const saveTask = async (task: Task) => setDoc(doc(db, getColName('tasks'), task.id), sanitizeData(task));
...
export const listenToAssets = (cb: (assets: Asset[]) => void) => {
    return onSnapshot(query(collection(db, getColName('assets'))), (s) => cb(snapToData<Asset>(s)));
};
...
export const saveAsset = async (asset: Asset) => {
    ...
    const dataToSave = sanitizeData({ ...asset, lastUpdated: new Date().toISOString() });
    ...
    if (isNew) {
        ...
        if (asset.assignedEmployee !== oldData.assignedEmployee) {
            if (asset.assignedEmployee && !oldData.assignedEmployee) await logAction(asset.id, 'Assigned', `Assigned to ${asset.assignedEmployee}`);
            else if (!asset.assignedEmployee && oldData.assignedEmployee) await logAction(asset.id, 'Returned', `Returned from ${oldData.assignedEmployee}`);
            else if (asset.assignedEmployee && oldData.assignedEmployee) await logAction(asset.id, 'Transferred', `Transferred from ${oldData.assignedEmployee} to ${asset.assignedEmployee}`);
        }
        else if (asset.location !== oldData.location) await logAction(asset.id, 'Transferred', `Moved from ${oldData.location} to ${asset.location}`);
        
        if (asset.status !== oldData.status) {
            if (asset.status === 'Retired') await logAction(asset.id, 'Retired', `Asset Retired`);
            else if (asset.status === 'Lost/Stolen') await logAction(asset.id, 'Updated', `Marked as Lost/Stolen`);
        }
    }
};
...
export const importAssetsBulk = async (newAssets: Asset[]) => {
    ...
    await batch.commit();
    await logAction('BULK', 'Created', `Imported ${newAssets.length} assets via Excel`);
    await createNotification('success', 'Bulk Import Successful', `Imported ${newAssets.length} assets into the registry.`);
};
...
const logAction = async (assetId: string, action: AssetLog['action'], details: string, documentId?: string, ticketId?: string) => {
    try {
        await addDoc(collection(db, getColName('logs')), {
            assetId, action, details, performedBy: currentUserProfile?.email || 'System',
            timestamp: new Date().toISOString(), documentId: documentId || null, ticketId: ticketId || null
        });
...
export const getStats = (assets: Asset[]): AssetStats => {
  ...
  const categoryCounts = config.categories.map(cat => ({
    name: cat, value: assets.filter(a => a.category === cat).length
  })).filter(c => c.value > 0);
...
export const listenToProjects = (cb: (projects: Project[]) => void) => {
    return onSnapshot(query(collection(db, getColName('projects'))), (s) => cb(snapToData<Project>(s)));
};
...
export const getOverdueItems = async (projects: Project[]): Promise<ProjectItem[]> => {
    ...
    projects.forEach(p => {
        if (p.status !== 'Completed') {
            p.items.forEach(i => {
                if (i.status !== 'Received' && i.dueDate && i.dueDate < today) overdue.push(i);
            });
        }
    });
    return overdue;
...
export const listenToIncidents = (cb: (incidents: IncidentReport[]) => void) => {
    return onSnapshot(query(collection(db, getColName('incidents'))), (s) => cb(snapToData<IncidentReport>(s)));
};

export const createIncidentReport = async (data: Omit<IncidentReport, 'id' | 'ticketNumber' | 'status' | 'createdAt'>) => {
    const id = 'inc-' + Date.now();
    const ticketNumber = `TKT-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`;
    const report: IncidentReport = { ...data, id, ticketNumber, status: 'New', createdAt: new Date().toISOString() };
    
    await setDoc(doc(db, getColName('incidents'), id), sanitizeData(report));
    
    if (data.assetId) {
        await updateDoc(doc(db, getColName('assets'), data.assetId), { status: 'Under Repair', lastUpdated: new Date().toISOString() });
        await logAction(data.assetId, 'Ticket', `Ticket ${ticketNumber} created: ${data.description}`, undefined, report.id);
    }

    const msg = `${data.priority} priority issue reported at ${data.location}: ${data.description}`;
    await createNotification('error', `New Ticket ${ticketNumber}`, msg, '/repairs');
    await sendSystemEmail(`New Ticket: ${ticketNumber}`, msg, `${window.location.origin}/#/repairs`);
};

export const updateIncidentReport = async (id: string, updates: Partial<IncidentReport>, autoFixAsset: boolean = false) => {
    ...
    if (incident.assetId) {
        if (['Open', 'In Progress', 'Waiting for Parts'].includes(updates.status || '')) {
            await updateDoc(doc(db, getColName('assets'), incident.assetId), { status: 'Under Repair' });
        } else if (updates.status === 'Resolved' && autoFixAsset) {
            await updateDoc(doc(db, getColName('assets'), incident.assetId), { status: 'Active' });
        }
        if (updates.status) await logAction(incident.assetId, 'Ticket', `Ticket ${incident.ticketNumber} updated to ${updates.status}`);
    }

    if (updates.status && incident.reporterEmail && updates.status !== incident.status) {
        const msg = `Your ticket for "${incident.assetName}" has been updated to: ${updates.status}.\n\nResolution Notes: ${updates.resolutionNotes || 'N/A'}`;
        await sendSystemEmail(`Update on Ticket ${incident.ticketNumber}`, msg, '', incident.reporterEmail);
    }
};
...
export const processAssetReplacement = async (ticketId: string, oldAssetId: string, newAssetId: string, notes: string) => {
    ...
    const batch = writeBatch(db);
    batch.update(doc(db, getColName('assets'), oldAssetId), { status: 'Retired', assignedEmployee: '', lastUpdated: new Date().toISOString() });
    batch.update(doc(db, getColName('assets'), newAssetId), { status: 'Active', location: incident.location, lastUpdated: new Date().toISOString() });
    batch.update(incidentRef, { status: 'Resolved', resolvedAt: new Date().toISOString(), resolutionNotes: `Asset Replaced. New Asset ID: ${newAssetId}. ${notes}` });
    await batch.commit();

    await logAction(oldAssetId, 'Retired', `Replaced by ${newAssetId}. Notes: ${notes}`, undefined, ticketId);
    await logAction(newAssetId, 'Replaced', `Replacement for ${oldAssetId}. Ticket ${incident.ticketNumber}`, undefined, ticketId);
    
    await createNotification('info', 'Asset Replaced', `Ticket ${incident.ticketNumber} resolved via replacement.`, '/repairs');
    if (incident.reporterEmail) await sendSystemEmail(`Ticket ${incident.ticketNumber} Resolved`, `Your issue has been resolved. The device was replaced.`, '', incident.reporterEmail);
};

// --- REQUESTS ---

export const listenToRequests = (cb: (reqs: AssetRequest[]) => void) => {
    return onSnapshot(query(collection(db, getColName('requests'))), (s) => cb(snapToData<AssetRequest>(s)));
};

export const createAssetRequest = async (data: Omit<AssetRequest, 'id' | 'requestNumber' | 'status' | 'createdAt'>) => {
    ...
    const req: AssetRequest = { ...data, id, requestNumber, status: 'New', createdAt: new Date().toISOString() };
    ...
    await sendSystemEmail(`New Asset Request: ${requestNumber}`, msg, `${window.location.origin}/#/requests`);
    if (data.requesterEmail) await sendSystemEmail(`Request Received: ${requestNumber}`, `We have received your request for ${data.category}.`, '', data.requesterEmail);
};
...
export const updateAssetRequest = async (id: string, updates: Partial<AssetRequest>) => {
    ...
    if (updates.status && req.requesterEmail && updates.status !== req.status) {
        await sendSystemEmail(`Request Update: ${req.requestNumber}`, `Your request for ${req.category} is now: ${updates.status}.\n\nNotes: ${updates.resolutionNotes || 'N/A'}`, '', req.requesterEmail);
    }
};

export const fulfillAssetRequest = async (id: string, assetId: string, notes: string, assignedTo: string) => {
    ...
    await logAction(assetId, 'Assigned', `Fulfilled Request ${req.requestNumber} for ${assignedTo}`);

    await updateDoc(reqRef, { status: 'Deployed', resolvedAt: new Date().toISOString(), linkedAssetId: assetId, resolutionNotes: notes });

    if (req.requesterEmail) await sendSystemEmail(`Request Fulfilled: ${req.requestNumber}`, `Good news! Your request for ${req.category} has been fulfilled and the asset has been deployed to ${assignedTo}.`, '', req.requesterEmail);
};
...
export interface PublicStatusResult {
  type: 'Ticket' | 'Request';
  id: string; status: string; created: string; subject: string; details: string; notes?: string; updated?: string;
}

export const getPublicItemStatus = async (refId: string): Promise<PublicStatusResult | null> => {
    const cleanId = refId.trim().toUpperCase();
    
    const incSnap = await getDocs(query(collection(db, getColName('incidents')), where('ticketNumber', '==', cleanId)));
    if (!incSnap.empty) {
        const data = incSnap.docs[0].data() as IncidentReport;
        return { type: 'Ticket', id: data.ticketNumber, status: data.status, created: data.createdAt, subject: data.assetName, details: data.description, notes: data.resolutionNotes, updated: data.resolvedAt };
    }

    const reqSnap = await getDocs(query(collection(db, getColName('requests')), where('requestNumber', '==', cleanId)));
    if (!reqSnap.empty) {
        const data = reqSnap.docs[0].data() as AssetRequest;
        return { type: 'Request', id: data.requestNumber, status: data.status, created: data.createdAt, subject: data.category, details: data.reason, notes: data.resolutionNotes, updated: data.resolvedAt };
    }
    return null;
};
...
export const listenToInvoices = (cb: (inv: Invoice[]) => void) => {
    return onSnapshot(query(collection(db, getColName('invoices'))), (s) => cb(snapToData<Invoice>(s)));
};
export const saveInvoice = async (invoice: Invoice) => setDoc(doc(db, getColName('invoices'), invoice.id), sanitizeData(invoice));
export const deleteInvoice = async (id: string) => deleteDoc(doc(db, getColName('invoices'), id));

export const getHandoverDocuments = async (): Promise<HandoverDocument[]> => snapToData<HandoverDocument>(await getDocs(collection(db, getColName('documents'))));
export const saveHandoverDocument = async (docData: HandoverDocument) => setDoc(doc(db, getColName('documents'), docData.id), sanitizeData(docData));

export const createPendingHandover = async (employeeName: string, assets: Asset[]): Promise<string> => {
    const id = 'ph-' + Date.now();
    await setDoc(doc(db, getColName('pendingHandovers'), id), sanitizeData({
        id, employeeName, assetIds: assets.map(a => a.id),
        assetsSnapshot: assets.map(a => ({ id: a.id, name: a.name, serialNumber: a.serialNumber })),
        createdAt: new Date().toISOString(), createdBy: currentUserProfile?.email || 'System', status: 'Pending'
    } as PendingHandover));
    return id;
};

export const getPendingHandover = async (id: string): Promise<PendingHandover | undefined> => {
...
export const completePendingHandover = async (id: string, signature: string) => {
    ...
    await saveHandoverDocument({
        id: 'doc-' + Date.now(), employeeName: pending.employeeName, assets: pending.assetsSnapshot,
        signatureBase64: signature, date: new Date().toISOString(), type: 'Handover'
    });
    ...
    await sendSystemEmail('Handover Signed', msg, `${window.location.origin}/#/staff`);
};

// --- BULK OPS ---

const performBulkOp = async (assetIds: string[], updateFn: (id: string) => object, logActionFn: (id: string) => Promise<void>) => {
    const batch = writeBatch(db);
    assetIds.forEach(id => batch.update(doc(db, getColName('assets'), id), updateFn(id)));
    await batch.commit();
...
export const bulkAssignAssets = async (assetIds: string[], employeeName: string) => {
    await performBulkOp(
        assetIds,
        () => ({ assignedEmployee: employeeName, status: 'Active', lastUpdated: new Date().toISOString() }),
...
export const bulkReturnAssets = async (assetIds: string[], docId?: string) => {
    await performBulkOp(
        assetIds,
        () => ({ assignedEmployee: '', status: 'In Storage', lastUpdated: new Date().toISOString() }),
...
export const renameMasterDataItem = async (type: 'category' | 'location' | 'department', oldValue: string, newValue: string) => {
    const config = await getAppConfig();
    if (type === 'category') config.categories = config.categories.map(c => c === oldValue ? newValue : c);
    else if (type === 'location') config.locations = config.locations.map(l => l === oldValue ? newValue : l);
    else if (type === 'department') config.departments = (config.departments || []).map(d => d === oldValue ? newValue : d);
    
    await saveAppConfig(config);

    const snapshot = await getDocs(query(collection(db, getColName('assets')), where(type, '==', oldValue)));
    const batch = writeBatch(db);
